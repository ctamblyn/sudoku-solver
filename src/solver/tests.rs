...
fn solves_completed_puzzles() {
    ...
    assert_eq!(solution.unwrap(), board);
...
#[test]
fn solution_iter_from_impl_works() {
    let board = Board::from(&[
        [9, 0, 6, 0, 7, 0, 4, 0, 3], // row 1
        [0, 0, 0, 4, 0, 0, 2, 0, 0], // row 2
        [0, 7, 0, 0, 2, 3, 0, 1, 0], // row 3
    ...
    let mut new_solutions = SolutionIter::new(&board);
    let mut from_solutions = SolutionIter::from(board);

    assert_eq!(new_solutions.next(), from_solutions.next());
    assert_eq!(new_solutions.next(), from_solutions.next());
    assert_eq!(new_solutions.next(), None);
    assert_eq!(from_solutions.next(), None);
}
